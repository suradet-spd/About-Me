...
use App\Http\Controllers\set_system_config;
// Use Model file
...
Route::get('/', function () {
    ...
    $tmp_system = systemConfig::all()->whereNull('exp_date')->toArray();
    $system = array(
    ...
    foreach ($tmp_system as $ts) {
        if ($ts["config_type"] == 'BG') {
            $system["background_color"] = json_decode(decrypt($ts["config_desc"]));
        } else if ($ts["config_type"] == 'FC') {
            $system["font_color"] = json_decode(decrypt($ts["config_desc"]));
        } else if ($ts["config_type"] == 'BC'){
            $system["button_color"] = json_decode(decrypt($ts["config_desc"]));
        } else if ($ts["config_type"] == 'AD'){
            $system["account_detail"] = json_decode(decrypt($ts["config_desc"]));
        }
    }
        ...
        if (!$req->exists('SearchTXT')) {
            ...
            if (!isset($list_profile)) {
                ...
                if ($list_profile == null or $list_profile == "") {
                    ...
                    return view('home', compact('list_profile' , 'system'));
                }
            }
        }
    }
...
Route::post('/system-config', [set_system_config::class, 'SetSystem'])->name('systemConfig');
